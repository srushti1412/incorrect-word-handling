...
def translate_to_french(english_sentence):
    ...
    return french_sentence.strip().replace("[start]", "").replace("[end]", "")

def translate_to_spanish(english_sentence):
    spanish_sentence = decode_sentence(english_sentence)
    return spanish_sentence.strip()

# Check if a word is valid and provide suggestions if not
def check_valid_words(sentence, valid_words):
    ...
    for word in words:
        if word not in valid_words:
            ...
            # Provide suggestions (for simplicity, just showing words starting with the same letter)
            suggestions[word] = [w for w in valid_words if w.startswith(word[0])]
    
    return invalid_words, suggestions
...
def handle_translate():
    ...
    valid_words = eng_vocab  # Assuming eng_vocab contains all valid English words
    invalid_words, suggestions = check_valid_words(english_sentence, valid_words)
    
    if invalid_words:
        error_message = f"Word(s) not available: {', '.join(invalid_words)}.\nSuggestions:\n"
        for word, suggestion in suggestions.items():
            error_message += f"{word}: {', '.join(suggestion)}\n"
        
        if len(invalid_words) > 1:
            error_message += f"List of wrong words entered so far: {', '.join(invalid_words)}"
        
        translation_output.delete("1.0", "end")
        translation_output.insert("end", error_message)
    else:
